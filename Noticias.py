...
def insertar_resultado(palabra, url, titulo, descripcion, fecha, conn):
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO searchapp_new (palabra, url, titulo, descripcion, fecha) VALUES (%s, %s, %s, %s, %s)",
                       (palabra, url, titulo, descripcion, fecha))
        conn.commit()
        cursor.close()
    except Exception as e:
...
def buscar_palabra(palabra, url, conn):
    ...
    if response.status_code == 200:
        ...
        if entry_header:
            titulo = entry_header.find('h1').text.strip()
            fecha = entry_header.find('div', class_='jeg_meta_date').text.strip()
        else:
            titulo = ""
            fecha = ""

        # Verifica si hay título, descripción y fecha antes de insertar
        if titulo and descripcion and fecha:
            if palabra in descripcion.lower() or palabra in titulo.lower():
                print(palabra, "url encontrada:", url)
                insertar_resultado(palabra, url, titulo, descripcion, fecha, conn)
            else:
                print("La palabra no se encontró en la descripción ni en el título de la URL:", url)
        else:
            print("No se encontraron datos suficientes en la URL para almacenar en la base de datos:", url)
